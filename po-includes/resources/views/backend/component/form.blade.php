<div class="form-group">
	...
	{!! Form::text('title', null, ['class' => $errors->has('title') ? 'form-control is-invalid' : 'form-control', 'required' => 'required', $formMode == 'edit' ? 'required' : '']) !!}
	{!! $errors->first('title', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-row">
	<div class="form-group col-md-4">
		{!! Form::label('author', __('component.author').' *', ['class' => 'control-label']) !!}
		{!! Form::text('author', null, ['class' => $errors->has('author') ? 'form-control is-invalid' : 'form-control', 'required' => 'required', $formMode == 'edit' ? 'required' : '']) !!}
		{!! $errors->first('author', '<p class="help-block">:message</p>') !!}
	</div>
	<div class="form-group col-md-4">
		{!! Form::label('folder', __('component.folder').' *', ['class' => 'control-label']) !!}
		{!! Form::text('folder', null, ['class' => $errors->has('folder') ? 'form-control is-invalid' : 'form-control', 'required' => 'required', $formMode == 'edit' ? 'required' : '']) !!}
		{!! $errors->first('folder', '<p class="help-block">:message</p>') !!}
	</div>
	<div class="form-group col-md-4">
		{!! Form::label('type', __('component.type').' *', ['class' => 'control-label']) !!}
		<select class="{{ $formMode == 'edit' ? '' : 'select-style' }} form-control" id="type" name="type" {{ $formMode == 'edit' ? 'required' : '' }}>
			@if($formMode == 'edit')
