@extends('layouts.app')
@section('title', __('runningtext.datatable_title'))
		...
		<div>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb breadcrumb-style1 mg-b-10">
					...
					<li class="breadcrumb-item"><a href="{{ url('/dashboard/runningtext/table') }}">{{ __('general.runningtext') }}</a></li>
					<li class="breadcrumb-item active" aria-current="page">{{ __('runningtext.datatable_list') }}</li>
				</ol>
			</nav>
			<h4 class="mg-b-0 tx-spacing--1">{{ __('runningtext.datatable_list') }}</h4>
		</div>

		<div>
			<a href="{{ url('dashboard/runningtext/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a>
		</div>
	</div>

	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/runningtext/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="runningtext-table">
					<thead>
						<tr>
							<th style="text-align:center;" width="15"></th>
							<th style="text-align:center;" width="25">{{ __('general.id') }}</th>
							<th>{{ __('runningtext.isitext') }}</th>
							<th>{{ __('runningtext.uname') }}</th>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#runningtext-table').DataTable({
			processing: true,
			serverSide: true,
			stateSave: true,
			responsive: {
			...
			ajax: '{{ url("dashboard/runningtext/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
