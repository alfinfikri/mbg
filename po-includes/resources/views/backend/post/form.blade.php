<div class="row">
	...
	<div class="col-md-3">
		<div class="form-group">
			...
			{!! Form::text('seotitle', null, ['class' => $errors->has('seotitle') ? 'form-control is-invalid' : 'form-control', 'required' => 'required', 'readonly' => 'readonly']) !!}
			{!! $errors->first('seotitle', '<p class="help-block">:message</p>') !!}
		</div>
		<div class="form-group">
			{!! Form::label('tanggal', 'Tanggal *', ['class' => 'control-label']) !!}
			<div class="input-group">
			 <input class="date form-control" type="text" name="tanggal" value="{{ !empty($post->tanggal) ? $post->tanggal : null }}" placeholder="{{ date('Y-m-d') }}" onkeydown="return false" required>
			</div>
		...
		<div class="form-group">
			...
			<select class="select-style form-control" id="category_id" name="category_id">
				@if($formMode == 'edit')
				<option value="{{ old('category_id', !empty($post->category_id) ? $post->category_id : null) }}"> {{ __('general.selected') }} {{ old('title', !empty($post->category->title) ? $post->category->title : null)  }} </option>
				@endif
		...
		<div class="col-md-5">
			<div class="form-group">
				...
				<div class="input-group">
					...
					<span class="input-group-append">
						<a href="{{ url('po-content/cms-pemkot/file/dialog.php?type=1&field_id=picture-url&relative_url=1&akey='.config('fm.key')) }}" id="filemanager-multi" class="btn btn-secondary"><i class="fa fa-file"></i> {{ __('general.browse') }}</a>
					</span>
				</div>
			</div>
		</div>
