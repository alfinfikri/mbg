@extends('layouts.app')
@section('title', __('infografis.datatable_title'))
		...
		<div>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb breadcrumb-style1 mg-b-10">
					...
					<li class="breadcrumb-item active" aria-current="page">{{ __('infografis.datatable_list') }}</li>
				</ol>
			</nav>
			<h4 class="mg-b-0 tx-spacing--1">{{ __('infografis.datatable_list') }}</h4>
		</div>

		<div>
			<a href="{{ url('dashboard/infografis/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a>
		</div>
	</div>

	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/infografis/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="infografis-table">
					<thead>
						<tr>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#infografis-table').DataTable({
			pageLength: 10,
			processing: true,
			serverSide: true,
			stateSave: true,
			...
			ajax: '{{ url("dashboard/infografis/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'id', name: 'infografis.id' },
				{ data: 'gambar', name: 'infografis.gambar' },
				{ data: 'created_at', name: 'infografis.created_at' },
