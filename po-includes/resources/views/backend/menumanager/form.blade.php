...
<div class="form-group">
	...
	<select class="form-control select-style" id="target" name="target">
		...
		<option {{old('target',$menumanager->target)=="none"? 'selected':''}} value="none"> None </option>
		<option {{old('target',$menumanager->target)=="_blank"? 'selected':''}} value="_blank"> _Blank </option>
		@endif
		@if($formMode == 'create')
		<option value="">--- Pilih Target ---</option>
