...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-20">
		<div>
			...
			<h4 class="mg-b-0 tx-spacing--1">LIST DELIVERY</h4>
		</div>
		
		<div><a href="{{ url('dashboard/deliverys/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a></div>
	</div>
	
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/deliverys/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="deliverys-table">
					<thead>
						<tr>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#deliverys-table').DataTable({
			processing: true,
			serverSide: true,
			stateSave: true,
			responsive: {
			...
			ajax: '{{ url("dashboard/deliverys/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'id', name: 'deliverys.id' },
				{ data: 'tanggal', name: 'deliverys.tanggal' },
				@if(!Auth::user()->hasRole('sppg'))
