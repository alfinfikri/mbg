@extends('layouts.app')
@section('title', __('banner.datatable_title'))
		...
		<div>
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb breadcrumb-style1 mg-b-10">
					...
					<li class="breadcrumb-item active" aria-current="page">{{ __('banner.datatable_list') }}</li>
				</ol>
			</nav>
			<h4 class="mg-b-0 tx-spacing--1">{{ __('banner.datatable_list') }}</h4>
		</div>

		<div>
			<a href="{{ url('dashboard/banner/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a>
		</div>
	</div>

	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/banner/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
				<input type="hidden" name="totaldata" id="totaldata" value="0" />
				<table class="table table-striped" id="banner-table">
					<thead>
						<tr>
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#banner-table').DataTable({
			...
			ajax: '{{ url("dashboard/banner/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'id', name: 'banner.id' },
				{ data: 'gambar', name: 'banner.gambar' },
				{ data: 'created_at', name: 'banner.created_at' },
