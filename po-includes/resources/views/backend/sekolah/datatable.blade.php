...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-20">
		<div>
			...
			<h4 class="mg-b-0 tx-spacing--1">LIST SCHOOL</h4>
		</div>
		
		<div><a href="{{ url('dashboard/sekolahs/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10"><i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}</a></div>
	</div>
	
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => 'dashboard/sekolahs/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
...
<script type="text/javascript">
	$(function() {
		...
		var table = $('#sekolahs-table').DataTable({
			processing: true,
			serverSide: true,
			stateSave: true,
			responsive: {
			...
			ajax: '{{ url("dashboard/sekolahs/data") }}',
			autoWidth: false,
			order: [[1, 'desc']],
			columnDefs: [{
				targets: 'no-sort',
			...
			columns: [
				...
				{ data: 'id', name: 'sekolahs.id' },
				{ data: 'nama', name: 'sekolahs.nama' },
				{ data: 'wilayah', name: 'sekolahs.wilayah' },
				{ data: 'jumlah_total', name: 'sekolahs.jumlah_total' },
				{ data: 'status_layanan', name: 'sekolahs.status_layanan' },
