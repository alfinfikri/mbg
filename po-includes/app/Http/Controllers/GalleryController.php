    ...
    public function index($seotitle)
    {
		...
		if($album) {
			...
			$gallerys = Gallery::leftJoin('users', 'users.id', 'gallerys.created_by')
				...
				->select('gallerys.*', 'albums.title as atitle', 'users.name', 'users.picture as userpicture')
			...
			if($seotitle == 'all') {
				...
				$gallerys = Album::leftJoin('users', 'users.id', 'albums.created_by')
					->select('albums.*', 'users.id','users.name','users.picture', 'albums.created_at as created_at')
					->where('active', '=', 'Y')
					->has('gallerys')
					->paginate(6);
