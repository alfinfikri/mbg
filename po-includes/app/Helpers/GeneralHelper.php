...
if (!function_exists('runningText')) {
    function runningText()
    {
        $oldest = Runningtext::orderBy('created_at', 'asc')->first();

        if ($oldest) {
            $selisihHari = Carbon::now()->diffInDays($oldest->created_at);
            if ($selisihHari >= 7) {
                Runningtext::truncate();
            }
        }

		$result = Runningtext::select('id','isitext','created_at')->limit(1)->get();
		return $result;
	}
}

if (!function_exists('harirunningText')) {
    function harirunningText()
    {
		$result = Carbon::now()->format('l, d F Y');
		return $result;
	}
}
...
if (!function_exists('getVisitorToday')) {
    ...
    {
		// $result = ViewPage::selectRaw('COUNT(DISTINCT ip) as count, DATE(created_at) as date')
		...
		// 	return '';
		// }
		$result = DB::table('visitor_day_counts')->select('visitor_count','visitor_day')
				  ->whereDate('visitor_day', now()->today())
				  ->first();
		if ($result) {
...
if (!function_exists('getVisitorMonth')) {
    ...
    {
		// $result = ViewPage::selectRaw('COUNT(DISTINCT ip) as count, MONTH(created_at) as month')
		...
		// 	return '';
		// }
		$result = DB::table('visitor_month_counts')->select('visitor_count','visitor_month')
			      ->where('visitor_month', date('Y-m'))
				  ->first();
		if ($result) {
			return $result->visitor_count;
		} else {
...
if (!function_exists('latestPostWithPaging')) {
	...
	{
		$query = Post::with(['category', 'createdBy'])
			->where('active', 'Y');
	...
	function popularPost($limit, $offset = '0', $date = 'all')
	{
		if($date == 'all') {
			$result = Post::leftJoin('users', 'users.id', 'posts.created_by')
				->leftJoin('categories', 'categories.id', 'posts.category_id')
				->where([['posts.active', '=', 'Y']])
				->select('posts.*', 'categories.title as ctitle', 'categories.seotitle as cseotitle', 'users.name', 'users.picture as userpicture')
...
//infografis
if (!function_exists('latestInfografis')) {
	function latestInfografis($limit)
	{
		$result = Infografis::leftJoin('users', 'users.id', 'infografis.created_by')
			->select('infografis.*')
...
if (!function_exists('banner')) {
	function banner($limit)
	{
		$result = Banner::leftJoin('users', 'users.id', 'banner.created_by')
			->select('banner.*')
