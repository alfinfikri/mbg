...
if (isset($_POST['name']))
{
	...
	$disallowed_exts = ['php','php3','php4','php5','php6','php7','php8','phps','pl','py','pyc','pyo','jsp','asp','htm','html','shtml','phtml','sh','cgi','phar'];
	$info = pathinfo($name);
	$extension = isset($info['extension']) ? strtolower($info['extension']) : '';

	if (in_array($extension, $disallowed_exts)) {
		response(trans('dangerous file type').AddErrorLocation())->send();
		exit;
	}

	// Cegah nama ganda: .php.jpg, .jpg.php
	if (preg_match('/\.(php[0-9]?|phtml|phar)(\..+)?$/i', $name)) {
		response(trans('wrong extension').AddErrorLocation())->send();
		exit;
	}

	if (is_function_callable('chmod') !== FALSE) {
    	chmod($path.$name, 0644); // pastikan tidak executable
	}
	...
	switch($_GET['action'])
	{
		...
		case 'create_file':
			...
			if (preg_match('/<\?(php)?/i', $content)) {
				response(trans('PHP code not allowed in content').AddErrorLocation())->send();
				exit;
			}
