...
if ( ! empty($_FILES) || isset($_POST['url']))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		// else $is_img=FALSE;
		$is_img = in_array($mime_type, ['image/png', 'image/jpeg', 'image/gif', 'image/webp']);
		if ($is_img && !@getimagesize($_FILES['file']['tmp_name'])) {
			unlink($_FILES['file']['tmp_name']);
			response(trans("Invalid image file").AddErrorLocation(), 406)->send();
			exit;
		}
		...
		if (preg_match('/<\?php/i', $file_contents)) {
			unlink($targetFile);
			response(trans("Malicious PHP content blocked").AddErrorLocation(), 406)->send();
			exit;
		}
