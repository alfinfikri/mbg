...
// Secure session start and header protections
session_start();
...
// Sanitize GET & POST input globally
function sanitize_input_array(array $input) {
    return array_map(function ($value) {
        return is_string($value) ? trim(htmlspecialchars($value, ENT_QUOTES, 'UTF-8')) : $value;
    }, $input);
}
$_GET = sanitize_input_array($_GET);
$_POST = sanitize_input_array($_POST);

// Whitelist actions (explicit allow list)
$allowed_actions = [
    'new_file_form', 'view', 'filter', 'sort', 'image_size', 'save_img',
    'extract', 'media_preview', 'copy_cut', 'clear_clipboard', 'chmod',
    'get_lang', 'change_lang', 'cad_preview', 'get_file'
];

// Block unexpected actions
...
if(isset($_GET['action']))
{
	switch($_GET['action'])
	{
		...
		case 'save_img':
			...
			if (!in_array(strtolower($info['extension']), $allowed_img_exts)) {
			...
			if (
				...
				|| $_POST['name'] !== fix_filename($_POST['name'], $config)
				|| ! preg_match('/\.(jpe?g|png)$/i', $_POST['name'])
				|| ! preg_match('/^[-\w]+\.(jpe?g|png)$/i', $_POST['name'])
			)
			{
				response(trans('wrong data').AddErrorLocation())->send();
				exit;
